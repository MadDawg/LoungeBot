...
class LoungeBot{
    ...
    constructSauceDispenser(results, indexes, minsim){
        ...
        for (let i = 0; i < results.length; i++){
            ...
            // also, sometimes the second result has a higher similarity value
            // so we'll test if that is valid then bail out if it isn't
            if (similarity < parseFloat(minsim)){
                if (i > 0) break; // bail out
                continue; // skip to next iteration (should always be index 1)
            }
    ...
    getSauce(message, args, manually_invoked=false, numres=2, minsim="65!"){
        ...
        if (manually_invoked){
            ...
            if (args.length > 1 || urls.length > 1){
                msg += "Note that only the first image/url in a message will be parsed.";
            }
            imgurl = urls[0];
        }
        else{
        ...
        https.get(url, (resp) => {
            ...
            if (resp.statusCode !== 200){
                if (resp.statusCode === 403){
                    this.logger.error("Server recieved invalid API key");
                    return;
                }
                else{
                    // TODO: contruct embed containing fallback_url
                    this.logger.error(`Server returned ${resp.statusCode} ${status_msg[String(resp.statusCode)]}`);
                    return;
                }
            }
