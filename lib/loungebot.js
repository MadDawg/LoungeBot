...
class LoungeBot{
    ...
    // TODO: DRY off guild queries. Seriously this is gross lol
    // Maybe use a Discord.Collection or similar like discordjs.guide does
    ...
    async changePrefix(newprefix, guildid){
        ...
        const oldprefix = guild[0].prefix;
    ...
    async addBotSpam(channels, guildid) {
        ...
        // making a copy seems to work better than trying to modify in-place
        const botspam = [...guild[0].botspam];
        const messages = [];
    ...
    async addAutoSauce(channels, guildid) {
        ...
        const autosauce = [...guild[0].autosauce];
        const messages = [];
        for(let i = 0; i < channels.length; i++){
            const channel = channels[i];
