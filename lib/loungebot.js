...
const { saucenao_indexes, imageboards, videos, communities } = require('./saucenao_objects.js');
...
class LoungeBot{
    ...
    getSauce(message, args, manually_invoked=false, numres=1, minsim="65!"){
        let imgurl = "";
        let msg = "";
        if (manually_invoked){
            imgurl = args[0];
            if (args.length > 1){
                msg += "Note that only the first image in a message will be parsed.";
            }
        }
        else{
