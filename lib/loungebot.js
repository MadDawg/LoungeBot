...
import { stream as _stream } from 'got';
...
import { BOT_COMMAND_PREFIX, BOT_TOKEN, SAUCENAO_API_KEY } from '../config.js';
...
class LoungeBot{
            ...
            if (communities.includes(result.header.index_id)){
                ...
                switch(result.header.index_id){
                    ...
                    case indexes['kemono']:
                        ...
                        embed.addField("Title", result.data.title);
                        embed.addField("User", result.data.user_name);
                        embed.addField("Service", result.data.service_name);
                        //embed.addField("Original Pixiv Fanbox source", result.data.ext_urls[0]);
                    break;

                    case indexes['skeb']:
                        embed.addField("Creator", result.data.creator_name);
                    break;
    ...
    async getSauce(message, options) {
        ...
        // TODO: figure out how to handle/log got errors
            ...
            resp.on('end', () => {
                ...
                if (header.status > 0){
                    ...
                    if (!results){
                        ...
                        if (options.manually_invoked){
                            return message.reply({ embeds: [new MessageEmbed().setDescription(user_error_msg)] });
                        }
                        return;
                    }
                }
                    ...
                    if (header.message){
                        ...
                        this.logger.warning(error_msg);
                        if (options.manually_invoked){
                            return message.reply({ embeds: [new MessageEmbed().setDescription(error_msg)] });
                        }
                        return;
                    }
